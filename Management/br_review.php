...
if (isset($_GET["request_id"])) {
	...
	$request_query = "select borrowings.borrowing_id ,
				borrowings.borrowing_finance_comment,
				borrowings.borrowing_admin_comment ,
				users.user_name,users.user_cpr ,
				 users.user_company , 
				 borrowings.borrowing_amount ,
				  borrowings.borrowing_status , 
				  borrowings.borrowing_request_date
				   from borrowings inner join users on users.user_cpr = borrowings.user_cpr where borrowings.borrowing_id = ? ;";
	...
	$borrowings_string = $con->prepare("select count(user_cpr) from borrowings where user_cpr = ? and borrowing_status = '1';");
	$borrowings_string->bind_param("i", $selected_user_cpr);
	$borrowings_string->execute();
	$borrowing_string_result = $borrowings_string->get_result();
	$num_borrowings = $borrowing_string_result->fetch_assoc();
...
<head>
	...
	<script src="/api/js/br_review.js"></script>
</head>
						...
						<div class="contact__form">
							...
							<div class="doctor__item__text">
								...
								<div class="col-lg-12 col-md-12">
									<ul>
										<li><i class="fa fa-clock-o"></i> الرقم
											الشخصي
											:
											<?php echo $user_data["user_cpr"]; ?>
										</li>
										...
										<li><i class="fa fa-money"></i> عدد
											السلفات
											الممنوحة :
											<?php echo $num_borrowings["count(user_cpr)"]; ?>
										</li>
									</ul>
								</div>
							</div>
							...
							<div class="review_form review_form--right">
								<div class="br_review">
									...
									<input type="text"  id="applicant" 
										...
										readonly>
									<input type="text" id="borrowing_id"
										value="<?php echo $request_data["borrowing_id"]; ?>"
										hidden>
									...
									<label for="amount">
										كمية السلفة </label>
									<input type="text" id="br_amount"
										value="<?php echo $request_data["borrowing_amount"] . "  د.ب"; ?>"
										readonly>
									...
									if ($user_type == 2) {
										...
										if ($request_data["borrowing_status"] <> 2) {

											echo " 
                                            							<label > نعليق أمين الصندوق </label>
                                            							<textarea id='br_finance_comment' readonly > $request_data[borrowing_finance_comment] </textarea>";
											echo " <button id = 'back' onclick='back()' name='requests_back'  class='site-btn' style='font-size:15px;background-color:rgba(6,6,6,0.9);'> الرجوع </button>";

										} else {
											echo "
												<label > تعليق أمين الصندوق </label>
												<textarea id='financeComment' placeholder = 'تعليق أمين الصندوق'></textarea>
												<div id = 'error' ></div>";
											echo "
												<button id = 'accept' onclick = 'financeBtnFun(event)' style='font-size:15px;background-color:rgba(4,170,109,0.9);'  class='site-btn' >رفع الطلب </button>
												<button id = 'reject' onclick = 'financeBtnFun(event)' style='font-size:15px;backhground-color:red;'  class='site-btn' >رفض الطلب</button>
										...
										if ($request_data["borrowing_status"] <> 3) {
											echo "  
                                            						<label > تعليق أمين الصندوق </label>
                                            						<textarea name='financeComment' readonly > $request_data[borrowing_finance_comment] </textarea>";
											echo " 
											<label > تعليق المدير </label>
											<textarea name='admin_comment' readonly > $request_data[borrowing_admin_comment] </textarea>";
											echo "<button id = 'back' onclick='back()'  class='site-btn' style='background-color:rgba(6,6,6,0.9);'> الرجوع </button>";

										} else {
											echo "
											<label > نعليق أمين الصندوق </label>
											<textarea name='finance_comment' readonly > $request_data[borrowing_finance_comment] </textarea>";
											echo "
											<label > نعليق المدير </label>
											<textarea id ='adminComment' placeholder = 'تعليق الإدارة' ></textarea>
											<div id = 'error' ></div>";
											echo "
											<button id = 'accept' onclick = 'adminBtnFun(event)'  style='font-size:15px;background-color:rgba(4,170,109,0.9);' class='site-btn' >قبول الطلب </button>
											<button id = 'reject' onclick = 'adminBtnFun(event)' style='font-size:15px;backhground-color:red;' type='submit' class='site-btn' >رفض الطلب</button>
											<button id = 'back' onclick='back()' type='submit' name='requests_back'  class='site-btn' style='font-size:15px;background-color:rgba(6,6,6,0.9);'> الرجوع </button>
                                                                        ";
